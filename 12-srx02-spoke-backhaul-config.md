...
| Hostname         | srx02              |
| Role             | AutoVPN Spoke      |
| fxp0 (mgmt)      | 172.27.1.51/24     |
| ge-0/0/0 (WAN)   | 10.0.1.2/30        |
| ge-0/0/1 (LAN)   | 192.168.2.1/24     |
| st0.0 (tunnel)   | unnumbered (P2P)   |
| WAN peer (CSR1)  | 10.0.1.1           |
| Hub WAN IP       | 10.0.0.2           |
...
srx02 is an AutoVPN spoke operating in **full-tunnel** mode: anything not local to its own
LAN (`192.168.2.0/24`) is sent up the tunnel to the hub via a default route on `st0.0`. The
...
> ```bash
> source secrets.env
> sed "s/\$AUTOVPN_PSK/$AUTOVPN_PSK/g" 12-srx02-spoke-backhaul-config.md > /tmp/srx02-deploy.md
> ```
> Apply from `/tmp/srx02-deploy.md`, then `rm /tmp/srx02-deploy.md`.
...
set interfaces fxp0 unit 0 family inet address 172.27.1.51/24

# --- WAN interface (P2P to CSR1 gi2) ---
...
set security ike gateway AUTOVPN-SPOKE-GW local-identity hostname srx02.homelab.local
...
set routing-options static route 10.0.0.0/30 next-hop 10.0.1.1
set routing-options static route 10.0.1.0/30 next-hop 10.0.1.1
...
# Routing — default should point at st0.0; 10.0.0.2/32 must remain via 10.0.1.1
...
# Spoke-to-spoke (once another spoke is up), backhauled via the hub
