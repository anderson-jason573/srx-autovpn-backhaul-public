# srx03 — AutoVPN Spoke Configuration (Full-Tunnel Backhaul)
## Junos OS 23.2R2.21 | VM ID 152 (rebuild on 23.2R2.21) | Role: AutoVPN Spoke
...
> (`03-srx03-spoke-config.md`) with a full-tunnel traffic selector and a default route into
...
| Hostname         | srx03              |
| Role             | AutoVPN Spoke      |
| fxp0 (mgmt)      | 172.27.1.52/24     |
| ge-0/0/0 (WAN)   | 10.0.2.2/30        |
| ge-0/0/1 (LAN)   | 192.168.3.1/24     |
| st0.0 (tunnel)   | unnumbered (P2P)   |
| WAN peer (CSR1)  | 10.0.2.1           |
| Hub WAN IP       | 10.0.0.2           |
...
srx03 is an AutoVPN spoke operating in **full-tunnel** mode: anything not local to its own
LAN (`192.168.3.0/24`) is sent up the tunnel to the hub via a default route on `st0.0`. The
...
- The host route to the hub WAN IP (`10.0.0.2/32 → 10.0.2.1`) is **retained and is now
...
> ```bash
> source secrets.env
> sed "s/\$AUTOVPN_PSK/$AUTOVPN_PSK/g" 13-srx03-spoke-backhaul-config.md > /tmp/srx03-deploy.md
> ```
> Apply from `/tmp/srx03-deploy.md`, then `rm /tmp/srx03-deploy.md`.

```
# ============================================================
# srx03 — AutoVPN Spoke (FULL-TUNNEL BACKHAUL)
# Junos OS 23.2R2.21
...
# --- System ---
set system host-name srx03
set system services ssh
set system services netconf ssh

# --- Management interface (out-of-band) ---
set interfaces fxp0 unit 0 family inet address 172.27.1.52/24

# --- WAN interface (P2P to CSR1 gi3) ---
...
set security ike gateway AUTOVPN-SPOKE-GW local-identity hostname srx03.homelab.local
...
set security ipsec vpn AUTOVPN-SPOKE traffic-selector TS-SPOKE local-ip 192.168.3.0/24
...
set routing-options static route 10.0.0.2/32 next-hop 10.0.2.1

# WAN underlay reachability (keeps physical/WAN traffic out of the tunnel)
set routing-options static route 10.0.0.0/30 next-hop 10.0.2.1
set routing-options static route 10.0.1.0/30 next-hop 10.0.2.1
set routing-options static route 10.0.2.0/30 next-hop 10.0.2.1
set routing-options static route 10.0.3.0/30 next-hop 10.0.2.1
...
# Routing — default should point at st0.0; 10.0.0.2/32 must remain via 10.0.2.1
show route 0.0.0.0/0
show route 10.0.0.2

# Internet backhaul through the hub (CSR1 loopback = simulated internet)
ping 10.100.100.1 source 192.168.3.1 count 5

# Spoke-to-spoke (e.g. to srx02), backhauled via the hub
ping 192.168.2.1 source 192.168.3.1 count 5
